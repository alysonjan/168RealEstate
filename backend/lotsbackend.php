...
if (isset($_POST['lots-save-btn'])) {
    ...
    if ($_POST["edit_property_id"] != '') {
       $query = "UPDATE lots SET property_name='$property',
       ...
       $query = "INSERT INTO lots (ImageId,main_image_id, property_name, locations, 
    ...
    if ($query_run) {
       ...
       for ($i = 0; $i < $total; $i++) {
          ...
          if ($tmpFilePath != "") {
             ...
             $newFilePath = 'lots/' . $_FILES['image']['name'][$i];
 
             //Upload the file into the temp dir
             if (move_uploaded_file($tmpFilePath, $newFilePath)) {
 
                //Handle other code here
                $query2 = "INSERT INTO lots_images (ImageId,images,filepath) VALUES ( '$propertyid','$picture','$newFilePath')";
    ...
    if($query_run2) {
      ...
      for ($i = 0; $i < $main_total; $i++) {
         ...
         if ($main_tmpFilePath != "") {
            ...
            //Setup our new file path
            $main_newFilePath = 'lotmain/'. $_FILES['main_image']['name'][$i];
           
            //Upload the file into the temp dir
            if (move_uploaded_file($main_tmpFilePath, $main_newFilePath)) {

               //Handle other code here
               $query3 = "INSERT INTO lots_main_image (main_image,filepath,image_id) 
...
if (isset($_POST['property_id'])) {
   ...
   $output = '';
   $query = "SELECT * FROM lots WHERE id = '" . $_POST["property_id"] . " ' ";
   $query_run = mysqli_query($connection, $query);
   $output .= '
...
if (isset($_POST['edit_property_id'])) {
   $query = "SELECT * FROM lots WHERE id = '" . $_POST["edit_property_id"] . " ' ";
   $query_run = mysqli_query($connection, $query);
...
if (isset($_POST['lots_delete_property_id'])) {
   $id = $_POST['lots_delete_property_id'];
   $query = "SELECT * FROM lots WHERE id = '$id' ";
   $query_run = mysqli_query($connection, $query);
   while ($row = mysqli_fetch_array($query_run)) {
      $id = $row['id'];

      echo '<input type="hidden" name="lots_delete_property_id" id="lots_delete_property_id" class="form-control" value=" ' . $row['id'] . ' ">
