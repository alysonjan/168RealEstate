...
if (isset($_POST['user_delete_property_id'])) {
   $id = $_POST['user_delete_property_id'];
   $query = "SELECT * FROM users WHERE id = '$id' ";
   $query_run = mysqli_query($connection, $query);
   while ($row = mysqli_fetch_array($query_run)) {
      $id = $row['id'];

      echo '<input type="hidden" name="user_delete_property_id" id="user_delete_property_id" class="form-control" value=" ' . $row['id'] . ' ">
...
if (isset($_POST['house-save-btn'])) {
   ...
   if ($_POST["edit_property_id"] != '') {
      $query = "UPDATE houseandlot SET developer='$developer',property_name='$property',
      ...
      $query = "INSERT INTO houseandlot (imageid,main_image_id,developer, property_name, locations, 
      ...
      VALUES ('$propertyid','$main_imageid', '$developer', '$property', '$location', '$units',
       '$squarefoot','$pricepersqft','$price', '$completiondate','$reservationfee','$yearbuilt',
       '$estimatedmonthly','$status','$description')";
   }
   ...
   if ($query_run) {
      ...
      for ($i = 0; $i < $total; $i++) {
         ...
         if ($tmpFilePath != "") {
            ...
            $newFilePath = 'houseandlot/'. $_FILES['image']['name'][$i];
           
            //Upload the file into the temp dir
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {

               //Handle other code here
               $query2 = "INSERT INTO houseandlot_images (imageid,images,filepath) 
               VALUES ('$propertyid','$picture','$newFilePath')";
   ...
   if($query_run2) {
      ...
      for ($i = 0; $i < $main_total; $i++) {
         ...
         if ($main_tmpFilePath != "") {
            ...
            //Setup our new file path
            $main_newFilePath = 'housemain/'. $_FILES['main_image']['name'][$i];
           
            //Upload the file into the temp dir
            if (move_uploaded_file($main_tmpFilePath, $main_newFilePath)) {

               //Handle other code here
               $query3 = "INSERT INTO houseandlot_main_image (main_image,filepath,image_id) 
...
if (isset($_POST['property_id'])) {
   ...
   $output = '';
   $query = "SELECT * FROM houseandlot WHERE id = '" . $_POST["property_id"] . " ' ";
   $query_run = mysqli_query($connection, $query);
   $output .= '
...
if (isset($_POST['edit_property_id'])) {
   $query = "SELECT * FROM houseandlot WHERE id = '" . $_POST["edit_property_id"] . " ' ";
   $query_run = mysqli_query($connection, $query);
...
if (isset($_POST['delete_property_id'])) {
   ...
   $query = "SELECT * FROM houseandlot WHERE id = '$id' ";
   $query_run = mysqli_query($connection, $query);
   while ($row = mysqli_fetch_array($query_run)) {
      $id = $row['id'];

      echo '<input type="hidden" name="delete_property_id" id="delete_property_id" class="form-control" value=" ' . $row['id'] . ' ">
