...
$sql = "SELECT h.id,
                hmi.main_image, h.property_name, h.locations, h.squarefoot, h.price, h.reservationfee,
                h.status FROM lots as h, lots_main_image as hmi
                WHERE h.main_image_id = hmi.image_id AND h.status ='rent' ORDER BY h.id desc LIMIT $start_from, $record_per_page ";
...
$page_query =
  "SELECT h.id,hmi.main_image, h.property_name, h.locations, h.price, h.reservationfee,
              h.status FROM lots as h, lots_main_image as hmi
              WHERE h.main_image_id = hmi.image_id AND h.status ='rent' ORDER BY h.id desc ";
